...
import {ApiDef_ServerInfo, BasicServerInfo, Const_ERROR, Const_OK, Default_ServerInfoNodePath, Response_ServerInfo, ServerInfoFirebaseState} from '@nu-art/thunderstorm-v2-shared';
import {DBDef_DeletedDoc} from '@nu-art/sync-manager-shared';
...
export class ModuleBE_ServerInfo_Class
	...
	pingFirestore = async () => {
		try {
			...
			const collection = firestore.getCollection(DBDef_DeletedDoc);
			await collection.query.custom({where: {}, limit: 1});
			return Const_OK;
		} catch (e: any) {
			this.logError(e);
			return Const_ERROR;
