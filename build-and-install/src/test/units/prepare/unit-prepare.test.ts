...
export const TestSuite_ProjectUnitPrepare: TestSuite<Input, Output> = {
	...
	testcases: [
		...
		() => {
			...
			return {
				description: 'Should override existing tsconfig.json and .eslintrc.json unless customESLintConfig = true',
				input: {
					units: [unit],
					before: () => {
						ensureDir(lib1.main.path);
		...
		() => {
			...
			return {
				description: 'Should skip .eslintrc.json copy when customESLintConfig = true',
				input: {
					units: [unit],
					before: () => {
						ensureDir(lib1.main.path);
		...
		() => {
			...
			return {
				description: 'Fails when customTSConfig=true and tsconfig.json is missing',
				input: {
					units: [unit],
					before: () => {
						ensureDir(lib1.main.path);
		...
		() => {
			...
			return {
				description: 'Should copy tsconfig.json into each source subfolder (main/test)',
				input: {
					units: [unit],
					before: () => {
						ensureDir(lib1.main.path);
		...
		() => {
			...
			return {
				description: 'Fails when customESLintConfig=true but .eslintrc.json is missing',
				input: {
					units: [unit],
					before: () => {
						ensureDir(lib1.main.path);
		...
		() => {
			...
			return {
				description: 'Fails if either ESLint or TSConfig is missing when both are custom',
				input: {
					units: [unit],
					before: () => {
						ensureDir(lib1.main.path);
		...
		() => {
			...
			return {
				description: 'Should copy tsconfig only into test folder if main is missing',
				input: {
					units: [unit],
					before: () => {
						ensureDir(lib1.test.path);
		...
		() => {
			...
			return {
				description: 'Should use pre-existing tsconfig when customTSConfig = true',
				input: {
					units: [unit],
					before: () => {
						ensureDir(lib1.main.path);
		...
		() => {
			...
			return {
				description: 'Should use pre-existing .eslintrc.json when customESLintConfig = true',
				input: {
					units: [unit],
					before: () => {
		...
		() => {
			...
			return {
				description: 'Should handle multiple units with mixed custom/default configs',
				input: {
					units: [unit1, unit2],
					before: () => {
						ensureDir(lib1.main.path);
	...
	processor: async (testCase) => {
		const {units, before} = resolveContent(testCase.input);
		const test = async () => {
			before();
			await Promise.all(units.map(u => u.prepare({projectRoot, baiDefaultsPath: pathTo_Default_BAI})));
		};
