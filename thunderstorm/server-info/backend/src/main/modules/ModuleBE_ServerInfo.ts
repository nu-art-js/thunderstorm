...
import {ApiDef_ServerInfo, BasicServerInfo, Const_ERROR, Const_OK, Default_ServerInfoNodePath, Response_ServerInfo, ServerInfoFirebaseState} from '@nu-art/thunderstorm-shared';
...
export class ModuleBE_ServerInfo_Class
	...
	pingFirestore = async () => {
		try {
			await ModuleBE_Firebase.createAdminSession().getFirestoreV3().firestore.listCollections();
			return Const_OK;
		} catch (e: any) {
			this.logError(e);
			return Const_ERROR;
