import { _className, AppToolsScreen, Button, ComponentSync, LL_H_C, LL_V_L, TS_AppTools, TS_JSONViewer, TS_TextArea } from "@nu-art/thunder-routing";
import { __stringify, exists, sortArray } from '@nu-art/ts-common';
import './ATS_AppConfigEditor.scss';
import { DB_AppConfig } from '@nu-art/thunderstorm-app-config-shared';
import { AppConfigKey_FE, ModuleFE_AppConfig } from '../../ModuleFE_AppConfig.js';
type Props = {
...
export class ATS_AppConfigEditor extends ComponentSync<Props, State> {
    //######################### Static #########################
    ...
    private render_AppConfigList = () => {
        ...
        return <LL_V_L className={'app-config-editor__config-list'}>
			{this.state.configs.map(config => {
                ...
                return <div key={config._id} className={className} onClick={() => this.selectConfig(config)}>{config.key}</div>;
            })}
		</LL_V_L>;
    };
    //######################### Render - Editor #########################
    ...
    private render_Editor_DataEditor = () => {
        ...
        return <LL_V_L className={'app-config-editor__editors'}>
			...
			<Button disabled={exists(error)} onClick={() => this.saveData(dataObject as {
            data: any;
        })}>Save Data</Button>
		</LL_V_L>;
    };
