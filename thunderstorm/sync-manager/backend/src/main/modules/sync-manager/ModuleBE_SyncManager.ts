...
import { __stringify, arrayToMap, currentTimeMillis, DB_Object, dispatch_onApplicationException, exists, filterDuplicates, filterInstances, LogLevel, Module, PreDB, ResolvableContent, resolveContent, RuntimeModules, TypedMap, UniqueId } from '@nu-art/ts-common';
import { firestore } from 'firebase-admin';
import { createBodyServerApi } from "@nu-art/thunder-db-api-backend/core/typed-api";
import { addRoutes } from "@nu-art/thunder-db-api-backend/modules/ModuleBE_APIs";
import { ModuleBE_BaseDB } from "@nu-art/thunder-db-api-backend/modules/db-api-gen/ModuleBE_BaseDB";
import { OnDatabaseChange } from '@nu-art/thunderstorm-backend/modules/db-api-gen/dispatchers';
import { ApiDef_SyncManager, DeltaSyncModule, FullSyncModule, LastUpdated, NoNeedToSyncModule, SmartSync_DeltaSync, SmartSync_FullSync, SmartSync_UpToDateSync, SyncDataFirebaseState, SyncManagerAPI_SmartSync } from '@nu-art/thunderstorm-sync-manager-shared';
import { DBDef_DeletedDoc, DBProto_DeletedDoc } from '@nu-art/thunderstorm-shared';
import { OnModuleCleanupV2 } from '@nu-art/thunderstorm-backend/_entity';
 ...
 * ```
 */
export class ModuleBE_SyncManager_Class extends Module<Config> implements OnModuleCleanupV2, OnDatabaseChange {
    ...
    __onItemsUpdated = async (collectionName: string, items: DB_Object | DB_Object[], latestUpdated: number, transaction?: Transaction) => {
        await this.setLastUpdated(collectionName, latestUpdated);
    };
    __onItemsDeleted = async (collectionName: string, items: DB_Object[] | null, uniqueKeys: string[], transaction?: Transaction) => {
        const now = currentTimeMillis();
        if (items === null) {
            ...
            await this.onItemsDeleted(collectionName, items, uniqueKeys, transaction);
