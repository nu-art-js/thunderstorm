...
export const ShowTooltipRelative = (content: () => JSX.Element, duration = -1, allowContentHover = false, delay = -1, relativeLocation?: {
	top?: number,
	right?: number
}) => {
	return {
		onMouseEnter: (e: React.MouseEvent<HTMLElement>) => {
			const data = e.currentTarget.getBoundingClientRect();
			const x = relativeLocation?.right != undefined ? data.right + relativeLocation.right : data.left + (data.width / 2);
			const y = relativeLocation?.top != undefined ? data.top + relativeLocation.top : (data.top + data.bottom) / 2;
			ModuleFE_Tooltip.showAt(content, x, y, duration, allowContentHover, 'right', delay);
		},
		onMouseLeave: (e: React.MouseEvent<any>) => ModuleFE_Tooltip.hide(duration),
	};
