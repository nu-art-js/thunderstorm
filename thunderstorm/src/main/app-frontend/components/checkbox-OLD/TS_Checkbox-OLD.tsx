...
export type CheckboxPropsOLD<T> = {
	key?: number | string
	id?: string
	value: T
	checked: boolean
...
export class TS_CheckboxOLD<T>
	extends React.Component<CheckboxPropsOLD<T>> {
	...
	render() {
		...
		return <div className={`${this.props.containerClass?.(this.props.checked, !!this.props.disabled)} ll_h_c`} style={radioContainer} id={this.props.id}
			...
			<div style={this.props.buttonClass ? {} : defaultButtonStyle}
					 className={this.props.buttonClass && this.props.buttonClass(this.props.checked, !!this.props.disabled)}>
	...
	renderLabel = () => <div
		className={'match_width'}>{typeof this.props.label === 'function' ? this.props.label(this.props.checked, !!this.props.disabled) : this.props.label}</div>;
}