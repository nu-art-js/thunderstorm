...
export class ATS_CrudOperations extends ComponentSync<{}, State> {
    //######################### Life Cycle #########################
    ...
    render() {
        ...
        return <LL_H_C className={'ats-crud-operations-container'}>
			<LL_V_L className={'ats-crud-operations-container__selection'}>
				...
				<TS_DropDown adapter={SimpleListAdapter(ACTIONS, action => <>{action.item.label}</>)} selected={selectedAction} placeholder={'Select Action'} onSelected={this.onDBActionSelected}/>
				<TS_TextArea key={'crud-operations-query'} className={'ats-crud-operations-container__selection__input'} type={'text'} placeholder={'Enter input'} disabled={false} value={input} onChange={this.onChangeInput} style={{ fontFamily: 'monospace', fontSize: 15 }}/>
				<LL_H_C className={'ats-crud-operations-container__selection__buttons'}>
					<Button className={'ats-crud-operations-container__selection__execute'} disabled={!(dbModuleToRequest && selectedAction && input)} onClick={this.onClickExecute} variant={'primary'}>Execute</Button>
    ...
    private renderResult = (item: any) => (<TS_TextArea key={item?._id} className={'ats-crud-operations-container__result'} type={'text'} value={JSON.stringify(item, null, 2)} placeholder={'Results'} disabled style={{ fontFamily: 'monospace', fontSize: 15 }}/>);
