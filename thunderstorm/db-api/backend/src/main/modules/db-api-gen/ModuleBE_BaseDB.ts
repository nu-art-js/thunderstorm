...
export abstract class ModuleBE_BaseDB<Proto extends DBProto<any>, ConfigType = any,
	...
	// not sure why this was here, but this is not in use in any of the projects
	...
	private _postWriteProcessing = async (data: PostWriteProcessingData<Proto['dbType']>, actionType: CollectionActionType, transaction?: Transaction) => {
		if (data.updated && !(Array.isArray(data.updated) && data.updated.length === 0)) {
			...
			await dispatch_onItemsUpdated.dispatchModuleAsync(this.dbDef.dbKey, data.updated, latestUpdated, transaction);
		}

		if (data.deleted && !(Array.isArray(data.updated) && data.updated.length === 0)) {
			await dispatch_onItemsDeleted.dispatchModuleAsync(this.dbDef.dbKey, asArray(data.deleted), this.config.uniqueKeys, transaction);
			...
			await dispatch_onItemsDeleted.dispatchModuleAsync(this.dbDef.dbKey, null, this.config.uniqueKeys, transaction);
