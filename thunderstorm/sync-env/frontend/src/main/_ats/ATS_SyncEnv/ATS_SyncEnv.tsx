...
import { _className, AppToolsScreen, ATS_Fullstack, Button, ComponentSync, genericNotificationAction, LL_H_C, LL_V_L, ModuleFE_BaseApi, ModuleFE_BaseDB, SimpleListAdapter, StorageKey, Thunder, TS_AppTools, TS_Checkbox, TS_DropDown, TS_Input, TS_PropRenderer } from '@nu-art/thunderstorm-frontend';
import { DBModuleType } from "@nu-art/thunder-db-api-shared";
import { ModuleFE_SyncManager } from '@nu-art/thunderstorm-sync-manager-frontend';
import { ModuleFE_SyncEnvV2 } from '../../modules/sync-env/ModuleFE_SyncEnvV2.js';
import { Response_FetchBackupMetadata } from '@nu-art/thunderstorm-sync-env-shared';
...
export class ATS_SyncEnvironment extends ComponentSync<{}, State> {
        ...
        return <LL_V_L className={'sync-env-page__menu'}>
			<LL_H_C className={'sync-env-page__menu__row'}>
				<TS_PropRenderer.Vertical label={'Environment'}>
					<TS_DropDown placeholder={'Select Environment'} className={'fancy'} adapter={envAdapter} onSelected={this.setEnv} selected={this.state.selectedEnv} canUnselect={true}/>
				</TS_PropRenderer.Vertical>
				<TS_PropRenderer.Vertical label={'Chunk Size'}>
					<TS_DropDown placeholder={'Chunk Size'} className={'fancy'} adapter={chunkSizesAdapter} onSelected={this.setChunkSize} selected={this.state.selectedChunkSize} canUnselect={true}/>
				</TS_PropRenderer.Vertical>
				{this.renderOperations()}
			</LL_H_C>
			<LL_H_C className={'sync-env-page__menu__row'}>
				<TS_PropRenderer.Vertical label={'Backup ID'}>
					<TS_Input className={_className(!tsValidateResult(this.state.backupId, tsValidateUniqueId) ? 'valid-id' : 'invalid-id')} type={'text'} value={this.state.backupId} onBlur={this.setBackupId}/>
				</TS_PropRenderer.Vertical>
				<TS_PropRenderer.Vertical label={'Collection Filter'}>
					<TS_Input onChange={val => this.setState({ searchFilter: val })} type={'text'} placeholder={'search collection'}/>
				</TS_PropRenderer.Vertical>
				{this.renderStatus()}
    ...
    private renderOperations = () => {
        return <TS_PropRenderer.Vertical label={'Operations'}>
			<LL_H_C className={'sync-env-page__operations'}>
				...
				{Thunder.getInstance().getConfig().name?.toLowerCase() === this.state.selectedEnv && <Button onClick={this.syncFirebase} disabled={!this.canSync()} className={'deter-users-from-this-button'}>Restore Firebase To Older Backup</Button>}
				<TS_Checkbox checked={this.state.cleanSync} onCheck={(value) => {
        ...
        return <>
			<LL_V_L className={'sync-env_modules-list-v2'}>
				<LL_H_C className={'utils'}>
					<TS_Checkbox checked={this.allModulesSelected()} onCheck={this.toggleSelectAll}>
						Select All
					</TS_Checkbox>
				</LL_H_C>
    ...
    private renderModule = (moduleName: string) => {
        ...
        return <TS_PropRenderer.Horizontal key={moduleName} label={<LL_H_C className={'header'}>
				<TS_Checkbox checked={this.state.selectedModules.has(moduleName)} onCheck={() => this.toggleSelectedModule(moduleName)}/>
				<div>{moduleName}</div>
			</LL_H_C>}>
			<LL_H_C className={'collection-row'}>
				...
				<div style={{ fontFamily: 'monospace' }}>{`${this.state.moduleList[moduleName].local ? 'L' : '-'}${this.state.moduleList[moduleName].remote
                ? 'R'
