import { _keys, ArrayType, AssetValueType, AwaitedDebounceInstance, compare, DBProto, deepClone, deleteKeysObject, exists, generateHex, InvalidResult, InvalidResultObject, isErrorOfType, KeysOfDB_Object, Logger, mergeObject, MUSTNeverHappenException, queuedDebounce, RecursiveReadonly, removeFromArrayByIndex, removeItemFromArray, ResolvableContent, resolveContent, Second, SubsetKeys, ValidationException, WhoCallThisException } from '@nu-art/ts-common';
 ...
 */
export class EditableItem<T> extends Logger {
 ...
 */
export class EditableDBItemV3<Proto extends DBProto<any>> extends EditableItem<Proto['uiType']> {
    private readonly module: ModuleFE_BaseApi<Proto>;
    // @ts-ignore
    private debounceInstance;
    ...
    updateItem(newItem: Proto['uiType']) {
        this.logWarning(`updating item externally, __updated: ${newItem.__updated}`);
