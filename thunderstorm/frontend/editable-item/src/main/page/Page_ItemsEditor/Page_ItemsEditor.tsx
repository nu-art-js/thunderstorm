...
import { _className, InferProps, InferState, LL_H_C, LL_H_T, LL_V_L, ModuleFE_BaseApi, ModuleFE_MouseInteractivity, mouseInteractivity_PopUp, openContent, ProtoComponent, ProtoComponentDef, SuperProto, TS_ButtonLoader, TS_Route } from '@nu-art/thunderstorm-frontend';
...
import { FrameLayout } from '@nu-art/thunderstorm-frontend/components/FrameLayout/index';
import { ModuleFE_BrowserHistoryV2 } from '@nu-art/thunderstorm-frontend/modules/ModuleFE_BrowserHistoryV2';
...
export abstract class Page_ItemsEditor<Proto extends DBProto<any>, CProto extends SuperProto<ProtoDef_Selection, ProtoComponentDef<string, any>> = ProtoDef_Selection, P = {}, S = {}> extends ProtoComponent<CProto, Props_ItemsEditor<Proto> & P, State_ItemsEditor<Proto> & S> {
        ...
        return <FrameLayout id={this.props.id} className="editor-page">
			<LL_H_T className={_className(this.props.className ?? 'editor-content', 'match_parent')}>
				<LL_V_L className="items-editor__list">
					...
					<List contextMenuItems={this.props.contextMenuActions} itemRenderer={this.props.itemRenderer} filter={this.state.filter} selected={selectedItem} sort={sort} module={this.props.module} onSelected={this.onSelected.bind(this)}/>
