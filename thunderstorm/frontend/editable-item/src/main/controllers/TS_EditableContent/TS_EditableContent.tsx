...
export abstract class TS_EditableContent<Opt, P = {}, S = {}, ItemType extends EditableContentType<Opt> = EditableContentType<Opt>, Props extends P & EditableRef<ItemType> & Props_Controller = P & EditableRef<ItemType> & Props_Controller, State extends S & _State<ItemType> = S & _State<ItemType>> extends ComponentSync<Props, State> {
    ...
    protected renderPropHorizontal(label: string, render: ReactNode, className?: string) {
        return (<TS_PropRenderer.Horizontal label={label} className={className}>
				{render}
