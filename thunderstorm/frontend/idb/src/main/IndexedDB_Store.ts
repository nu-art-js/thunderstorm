...
import {IDB_Config, IDb_Query, IDB_ReduceFunction} from './types.js';


type StoreResolver<Proto extends DBProto<any>> = (dbConfig: IDB_Config<Proto>, write?: boolean, store?: IDBObjectStore) => Promise<IDBObjectStore>;
type StoreExistsResolver<Proto extends DBProto<any>> = (dbConfig: IDB_Config<Proto>) => Promise<boolean>;

export class IndexedDB_Store<Proto extends DBProto<any>>
	extends Logger {

	private config: IDB_Config<Proto>;
	private storeResolver: StoreResolver<Proto>;
	private storeExistsResolver: StoreExistsResolver<Proto>;

	// ######################## Init ########################

	constructor(config: IDB_Config<Proto>, storeResolver: StoreResolver<Proto>, storeExistsResolver: StoreExistsResolver<Proto>) {
	...
	private getCursor = async (query?: IDb_Query): Promise<IDBRequest<IDBCursorWithValue | null>> => {
		const store = await this.getStore();
	...
	public async query(query: IDb_Query): Promise<Proto['dbType'][] | undefined> {
		const store = await this.getStore();
	...
	public async queryFilter(filter: (item: Proto['dbType']) => boolean, query?: IDb_Query): Promise<Proto['dbType'][]> {
	...
	public async WIP_queryMapNew<Type>(mapper: (item: Proto['dbType']) => Type, filter?: (item: Proto['dbType']) => boolean, query?: IDb_Query): Promise<Type[]> {
		const limit = query?.limit || 0;
		const cursorRequest = await this.getCursor(query);
		const matches: Type[] = [];
	...
	public async WIP_queryMap<Type>(mapper: (item: Proto['dbType']) => Type, filter?: (item: Proto['dbType']) => boolean, query?: IDb_Query): Promise<Type[]> {
		const limit = query?.limit || 0;
		const cursorRequest = await this.getCursor(query);
		const matches: Type[] = [];
	...
	public async queryReduce<ReturnType>(reducer: IDB_ReduceFunction<Proto['dbType'], ReturnType>, initialValue: ReturnType, filter?: (item: Proto['dbType']) => boolean, query?: IDb_Query) {
