...
export class IndexedDB_Database
	...
	registerStore = (dbConfig: IDB_Config<any>, onDBOpenCallback?: AsyncVoidFunction) => {
	...
	async getStore(config: IDB_Config<any>, write = false, _store?: IDBObjectStore): Promise<IDBObjectStore> {
		if (_store)
			return _store;
