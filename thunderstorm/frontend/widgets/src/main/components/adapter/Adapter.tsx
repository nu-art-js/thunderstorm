...
export type TreeType<TreeMap> = {
    map: TreeMap;
    action: {
        [K in keyof TreeMap]: (item: TreeMap[K]) => Promise<any> | any;
    };
    nodeRenderer: {
        [K in keyof TreeMap]: React.ComponentType<NodeRendererProps<TreeMap[K]>>;
    };
    renderer: {
        [K in keyof TreeMap]: React.ComponentType<{
            item: TreeMap[K];
        }>;
    };
    rendererV3: {
        [K in keyof TreeMap]: React.ComponentType<TreeMap[K]>;
    };
    nodeTypesMap: {
        [K in keyof TreeMap]: {
            type: K;
...
export class Adapter<T = any, I extends NodeRendererProps<T> = NodeRendererProps<T>> extends BaseAdapter<I> {
...
abstract class BaseAdapterBuilder<Data> {
    ...
    protected defaultTreeNodeRenderer = (props: NodeRendererProps) => {
        ...
        return (<div className="ll_h_c clickable" onClick={props.node.expandToggler}>
...
class ListSingleAdapterBuilder<ItemType = any> extends BaseAdapterBuilder<AdapterData<ListData<ItemType>>> {
...
class MultiTypeAdapterBuilder<TreeMap extends TreeType<any>> extends BaseAdapterBuilder<TreeMap['nodeType']> {
...
class TreeSingleAdapterBuilder<RenderItemType = any> extends BaseAdapterBuilder<AdapterData<TreeData<RenderItemType>>> {
