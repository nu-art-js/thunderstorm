...
export type BaseAppLevelProps_TS_TextAreaV2 = ComponentProps_Error & Omit<HTMLProps<HTMLTextAreaElement>, 'onChange' | 'onBlur' | 'ref'> & BaseInfraProps_TS_TextAreaV2 & {
    id?: string;
    placeholder?: string;
    name?: string;
    focus?: boolean;
...
export type NativeProps_TS_TextAreaV2 = Props_TS_TextAreaV2 & {
    componentRef?: React.RefObject<TS_TextAreaV2>;
};
/**
 ...
 *
 */
export class TS_TextAreaV2 extends React.Component<Props_TS_TextAreaV2, InputState> {
    ...
    render() {
        ...
        return <textarea {...props} {...convertToHTMLDataAttributes(this.props.error, 'error')} ref={input => {
            ...
            }} autoFocus={focus} onBlur={(event) => {
                const value = event.target.value;
                this.setState({ value });
                props.onBlur?.(value, event);
            }} name={props.name || props.id} className={_className('ts-textarea', props.className, this.props.resizeWithText && 'no-resize')} value={this.state.value} onChange={this.changeValue} onKeyDown={this.onKeyDown} autoComplete={props.autoComplete ? 'on' : 'off'}/>;
    }
}
