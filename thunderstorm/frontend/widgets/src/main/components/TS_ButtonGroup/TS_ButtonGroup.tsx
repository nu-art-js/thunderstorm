...
export class TS_ButtonGroup<ButtonKey extends string = string> extends ComponentSync<ButtonGroup_Props<ButtonKey>, ButtonGroup_State<ButtonKey>> {
    ...
    private renderButton = (button: ButtonGroupItem<ButtonKey, boolean>, index: number) => {
        ...
        return <React.Fragment key={button.key}>
			...
			<Button variant={'button-group-button'} className={buttonClassName} id={`button-${button.key}`} onClick={e => this.handleClick(e, button.key)} disabled={disableButtons}>
				{button.label}
			</Button>
		</React.Fragment>;
    };
