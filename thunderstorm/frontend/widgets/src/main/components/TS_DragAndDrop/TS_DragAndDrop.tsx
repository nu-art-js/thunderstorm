...
const DefaultFilesRenderer = (props: {
    acceptedFiles: File[];
    rejectedFiles: File[];
}) => {
    return <LL_V_L>
...
export class TS_DragAndDrop extends ComponentSync<Props_DragAndDrop, State> {
    static defaultProps = {
    ...
    static validateFilesBySuffix = (fileExt: string | string[]) => (files: File[]): DND_File[] => {
    ...
    render() {
        ...
        return (<LL_VH_C className={_className('ts-drag-and-drop', this.props.className)} id={this.props?.id}>
				<div className={`ts-drag-and-drop__content ts-drag-and-drop__${DND_Styles[this.state.dndState]}`} onDrop={this.onDrop} onDragEnter={this.onDragEnter} onDragOver={this.onDragOver} onDragLeave={this.onDragLeave} onClick={this.openFileChooser}>
					...
					<Renderer acceptedFiles={this.state.acceptedFiles} rejectedFiles={this.state.rejectedFiles}/>
				</div>
			</LL_VH_C>);
    }
