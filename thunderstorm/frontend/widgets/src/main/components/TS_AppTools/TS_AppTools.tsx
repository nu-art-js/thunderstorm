...
import { TS_NavLink } from '@nu-art/thunder-routing/ModuleFE_RoutingV2.js';
...
export class TS_AppTools extends React.Component<{}, State> {
    // ######################### Static #########################
    ...
    private renderHeader = () => {
        return <LL_H_C className={'ts-app-tools__header'}>
			<TS_Icons.menu.component className={'ts-app-tools__header__caret'} onClick={this.toggleNavBarCollapse}/>
    ...
    private renderNavbar = () => {
        ...
        return <LL_V_L className={className}>
			{groups.map(group => {
                return <TS_CollapsableContainer key={group.label} headerRenderer={group.label} containerRenderer={group.screens.map(this.renderNavbarItem)} customCaret={<TS_Icons.treeCollapse.component />} flipHeaderOrder={true} onCollapseToggle={() => this.toggleGroupCollapse(group.label)} collapsed={this.state.groups[group.label]}/>;
            })}
		</LL_V_L>;
    };
    private renderNavbarItem = (screen: AppToolsScreen) => {
        ...
        return <TS_NavLink key={screen.key} route={route} className={({ isActive }) => _className('ts-app-tools__nav-bar__item', isActive ? 'selected' : undefined)}>
			<Icon />
