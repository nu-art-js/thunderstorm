...
export class TS_ProgressBar extends ComponentSync<Props, State> {
    ...
    private renderRadialBar = () => {
        ...
        return <div className={'ts-progress-bar__radial-bar'}>
			...
			<svg className="ts-progress-bar__radial-bar__bar" viewBox={'0 0 100 100'}>
                ...
                return <circle key={i} cx="50" cy="50" r={radius} className={`ts-progress-bar__radial-bar__bar-child-${i}`} strokeDasharray={strokeDashArray} strokeDashoffset={strokeDashOffset}/>;
            })}
			</svg>
		</div>;
    };
