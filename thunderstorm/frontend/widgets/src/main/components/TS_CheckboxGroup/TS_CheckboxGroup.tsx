...
export class TS_CheckboxGroup extends ComponentSync<Props_CheckboxGroup, State_CheckboxGroup> {
    ...
    render() {
        ...
        return (<LL_V_L className={_className('ts-checkbox-group', className)} id={this.props.id}>
				<TS_Checkbox checked={allSelected} className={_className('ts-checkbox-group__parent', someSelected && 'ts-checkbox-group__partial')} disabled={parent.disabled} onCheck={this.onClickFather}>
					{parent.label}
				</TS_Checkbox>
				<LL_V_L className={'ts-checkbox-group__children'}>
					{options.map(option => (<TS_Checkbox key={option.id} checked={selectedIds.includes(option.id)} disabled={option.disabled || parent.disabled} onCheck={() => this.onClickCheckbox(option.id)}>
							{option.label}
						</TS_Checkbox>))}
				</LL_V_L>
			</LL_V_L>);
