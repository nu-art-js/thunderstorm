...
import { Adapter, TreeNode } from '../adapter/Adapter.js';
...
type State_Tree = {
    ...
    selected: {
        path?: string;
        item?: (item: any) => boolean;
    };
    adapter: Adapter;
};
const ignoreToggler = (): void => {
};
export class TS_Tree<P extends Props_Tree = Props_Tree, S extends State_Tree = State_Tree> extends ComponentSync<P, State_Tree> {
    // ######################## Static ########################
    static defaultProps: Partial<Props_Tree> = {
        checkExpanded: (expanded: TreeNodeExpandState, path: string) => expanded[path]
    };
    protected containerRefs: {
        [k: string]: HTMLDivElement;
    } = {};
    protected rendererRefs: {
        [k: string]: HTMLDivElement;
    } = {};
    // ######################## Life Cycle ########################
    constructor(props: P) {
    ...
    private renderChildren(data: any, nodePath: string, _path: string, level: number, filteredKeys: any[], renderChildren: boolean, adjustedNode: {
        ...
        return (<div className="ts-tree__children-container" ref={containerRefResolver}>
				{containerRef && filteredKeys.map((childKey) => this.renderNode(data[childKey], childKey, nodePath + (adjustedNode.deltaPath ? adjustedNode.deltaPath + '/' : ''), level + 1))}
			</div>);
    }
    private renderItem(item: any, path: string, key: string, nodeRefResolver: (_ref: HTMLDivElement) => void, level: number, isSelected: boolean, expanded?: boolean) {
        ...
        return <div tabIndex={this.props.indexTreeNodes ? 1 : undefined} data-path={path} className={className} ref={nodeRefResolver} onClick={this.onNodeClicked} onContextMenu={this.onContextMenuClicked}>
