import { _values, DB_Object, DBProto, Filter, ResolvableContent, resolveContent, sortArray } from '@nu-art/ts-common';
import * as React from 'react';
import { CSSProperties } from 'react';
import { TS_DropDown } from '../TS_Dropdown/index.js';
import { ModuleFE_BaseApi } from '@nu-art/thunder-db-api-frontend/ModuleFE_BaseApi.js';
...
export type BaseAppLevelProps_TS_GenericDropDownV3<T> = BaseInfraProps_TS_GenericDropDownV3<T> & {
    inputValue?: string;
...
export type TemplatingProps_TS_GenericDropDown<Proto extends DBProto<any>, T extends Proto['dbType'] = Proto['dbType']> = BaseInfraProps_TS_GenericDropDownV3<T> & {
    placeholder: string;
...
export type GenericDropDown_DBPointer_Item<Proto extends DBProto<any>> = {
    dbKey: Proto['dbKey'];
    item: Proto['dbType'];
};
export type TemplatingProps_TS_GenericDropDown_DBPointer<Proto extends DBProto<any>, T extends GenericDropDown_DBPointer_Item<Proto> = GenericDropDown_DBPointer_Item<Proto>> = Omit<BaseInfraProps_TS_GenericDropDownV3<T>, 'mapper'> & {
    placeholder: string;
    selectedItemRenderer?: (selected: T) => React.ReactNode;
    pointerProps: {
...
type Props_TS_GenericDropDownV3<Proto extends DBProto<any>, T extends Proto['dbType'] = Proto['dbType']> = TemplatingProps_TS_GenericDropDown<T> & BaseAppLevelProps_TS_GenericDropDownV3<T> & Props_CanUnselect<T> & {
...
export class GenericDropDownV3<Proto extends DBProto<any>, T extends Proto['dbType'] = Proto['dbType']> extends ComponentSync<Props_TS_GenericDropDownV3<Proto>, State<T>> {
    static readonly prepareSelectable = <Proto_ extends DBProto<any>>(mandatoryProps: ResolvableContent<TemplatingProps_TS_GenericDropDown<Proto_>>) => {
        return (props: AppLevelProps_TS_GenericDropDownV3<Proto_['dbType']>) => <GenericDropDownV3<Proto_> {...resolveContent(mandatoryProps)} {...props}/>;
    };
    ...
    static readonly prepareSelectable_DBPointers = <Proto_ extends DBProto<any>>(mandatoryProps: ResolvableContent<TemplatingProps_TS_GenericDropDown_DBPointer<Proto_>>) => {
        ...
        return (props: AppLevelProps_TS_GenericDropDownV3<GenericDropDown_DBPointer_Item<Proto_>>) => <GenericDropDownV3 {..._mandatoryProps} {...props} itemResolver={() => {
            ...
            }} renderer={(item: GenericDropDown_DBPointer_Item<Proto_>) => _mandatoryProps.pointerProps[item.dbKey].renderer(item)} mapper={(item: GenericDropDown_DBPointer_Item<Proto_>) => _mandatoryProps.pointerProps[item.dbKey].mapper(item)} 
        // @ts-ignore
        module={undefined}/>;
    };
    ...
    render() {
        return <TS_DropDown<T> ref={this.props.dropdownRef} // allow interacting with the TS_Input interface itself and not the html element inside
         id={this.props.id} innerRef={this.props.innerRef} tabIndex={this.props.tabIndex} error={this.state.error} className={this.props.className} placeholder={this.props.placeholder || 'Choose one'} inputValue={this.props.inputValue} adapter={this.state.adapter} filter={this.state.filter} selected={this.state.selected} onNoMatchingSelectionForString={this.props.onNoMatchingSelectionForString} onSelected={this.props.onSelected} noOptionsRenderer={this.props.noOptionsRenderer} caret={this.props.caret} selectedItemRenderer={this.props.selectedItemRenderer} boundingParentSelector={this.props.boundingParentSelector} renderSearch={this.props.renderSearch} limitItems={this.props.limitItems} hidePlaceholderOnOpen={this.props.hidePlaceholderOnOpen} unselectLabel={this.props.unselectLabel} canUnselect={this.props.canUnselect as (typeof this.props.canUnselect extends true ? true : false | undefined)} disabled={this.props.disabled}/>;
    }
}
