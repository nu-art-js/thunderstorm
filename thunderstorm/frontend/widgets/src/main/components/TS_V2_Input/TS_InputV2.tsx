...
export type BaseAppLevelProps_TS_InputV2 = ComponentProps_Error & Omit<HTMLProps<HTMLInputElement>, 'onChange' | 'onBlur' | 'ref'> & BaseInfraProps_TS_InputV2 & {
    id?: string;
    placeholder?: string;
    name?: string;
    focus?: boolean;
 ...
 *
 */
export class TS_InputV2 extends React.Component<Props_TS_InputV2, InputState> {
    ...
    render() {
        ...
        return <input {...props} {...convertToHTMLDataAttributes(this.props.error, 'error')} {...getErrorTooltip(this.props.error, this.props.showErrorTooltip)} autoFocus={focus} ref={this.inputRef} onBlur={(event) => {
                const value = event.target.value;
                this.setState({ value, focused: false });
                props.onBlur?.(value, event);
            }} onFocus={(event) => {
                this.setState({ focused: true });
            }} name={props.name || props.id} className={_className('ts-input', props.className)} value={this.state.value} onChange={this.changeValue} onKeyDown={this.onKeyDown} autoComplete={props.autoComplete ? 'on' : 'off'}/>;
    }
}
