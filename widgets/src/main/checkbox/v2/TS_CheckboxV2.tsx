...
export type IconSpec_TS_CheckboxV2 = {
...
export type Props_TS_CheckboxV2 = {
	...
	className?: string;
	id?: string;
	customIcons?: Partial<IconSpec_TS_CheckboxV2>;
};
type State = {
	checked: boolean | undefined;
	label: string;
...
export class TS_CheckboxV2
	...
	protected deriveStateFromProps(nextProps: Props_TS_CheckboxV2, state: State) {
	...
	private resolveIconNode = () => {
		...
		return exists(this.state.checked)
			? (this.state.checked
				? spec.checked
				: spec.unChecked) : spec.indeterminate;
	};
	...
	private render_Input = () => {
		...
		return <input type={'checkbox'} id={this.state.id} disabled={this.state.disabled} onChange={this.onChange} checked={!!this.state.checked}
