...
import './ToasterPortal_TopDown.scss';
...
export class ToasterPortal_TopDown
	...
	protected deriveStateFromProps(nextProps: InferProps<this>, state: InferState<this>) {
		...
		state.topOffset = nextProps.topOffset ?? 0;
		return state;
	}
	...
	private processItems() {
		...
		if (nextVisibleItem.model.body) {
			...
			if (element) {
				const maxHeight = this.state.height - this.state.topOffset;
