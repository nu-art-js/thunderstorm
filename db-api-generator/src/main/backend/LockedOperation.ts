...
export async function lockedOperationV2<T>(key: string, timeout: number, action: () => Promise<T>, exception: CustomException = new OperationLockedException('This action is already in progress')): Promise<T> {
	...
	const transactionResult = await ref.transaction(current => {
		...
		if (current && current.status === 'LOCKED' && now < current.timeout) {
			return current; // Returning current to keep the existing value
		}
		// Lock is either not held or expired, so we can acquire it
		return {status: 'LOCKED', timeout: now + timeout};
	});

// Check if the transaction was committed and lock was acquired
	if (!transactionResult.committed || transactionResult.snapshot.val().status !== 'LOCKED')
		throw exception; // Lock could not be acquired

	try {
		return await action();
	} finally {
		await ref.patch({status: 'UNLOCKED'}); // Setting the status to 'UNLOCKED'
	}
}