...
import { Adapter, SimpleTreeAdapter, ModuleFE_Toaster, TS_Tree } from "@nu-art/thunder-db-api-frontend/index";
import { __stringify, deepClone } from '@nu-art/ts-common';
type Type = {
    label: string | number;
    other: string;
};
class Example_Tree_SingleType_Renderer extends Component<{}> {
    static index = 0;
    private data: any;
    constructor(props: {}) {
        super(props);
    ...
    render() {
        const adapter: Adapter = SimpleTreeAdapter(this.data, (props: {
            item: Type;
        ...
        return <div>
			<div>
				...
				<TS_Tree adapter={adapter} onNodeClicked={(path: string, item: any) => ModuleFE_Toaster.toastInfo(`clicked on ${path}: ${__stringify(item)}`)}/>
			</div>
		</div>;
    }
