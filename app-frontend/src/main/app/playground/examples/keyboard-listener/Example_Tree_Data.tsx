...
class Example_Tree_Data extends ComponentSync<{}, State> {
    ...
    render() {
        ...
        return <LL_V_C style={{ minWidth: 180 }}>
			<TS_Tree adapter={adapter} onNodeFocused={(path: string) => this.setState({ actionMessage: `on focused: ${path}` })} onNodeClicked={(path: string) => this.setState({ actionMessage: `on clicked: ${path}` })}/>
...
class Example_ColorfulNodeRenderer extends React.Component<NodeRendererProps> {
    ...
    render() {
        ...
        return (<div className="ll_h_c" style={{ fontSize: '0.9em', lineHeight: 1.25 }}>
