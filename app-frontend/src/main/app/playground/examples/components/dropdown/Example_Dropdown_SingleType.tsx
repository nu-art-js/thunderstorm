...
class Example_Dropdown_SingleType extends ComponentSync<{}, State> {
    ...
    private renderDropDown() {
        ...
        return <div className="ll_v_l" style={{ marginLeft: 8 }}>
			<TS_DropDown placeholder="Choose" adapter={simpleAdapter} onSelected={this.onSelected} selectedItemRenderer={valueRenderer} filter={new Filter<Plague>((item: any) => ([item.label.toLowerCase()]))} caret={caret}/>
		</div>;
    }
    private renderConfigPanel() {
        return <div className="ll_v_l">
			<TS_Checkbox checked={this.state.autoComplete} onCheck={(autoComplete: boolean) => this.setState({ autoComplete })}/>

			<TS_Checkbox checked={this.state.filter} onCheck={(filter: boolean) => this.setState({ filter })}/>
		</div>;
    }
...
export class ItemRenderer extends BaseNodeRenderer<Plague> {
    renderItem(item: Plague) {
        return (<div className="ll_h_c clickable match_width">
