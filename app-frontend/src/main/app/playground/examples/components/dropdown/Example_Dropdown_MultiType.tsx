...
export class Example_Dropdown_MultiType extends React.Component<{}, {
    _selected?: Plague;
}> {
    ...
    render() {
        ...
        return <LL_V_C>
			...
			<TS_DropDown adapter={adapter} onSelected={this.onSelected} selectedItemRenderer={valueRenderer} inputEventHandler={(_state, e) => {
                if (e.key === 'Enter') {
                ...
                return _state;
            }} filter={new Filter<TreeItem<Plague>>(item => ([item.item.label.toLowerCase()]))} selected={flatPlaguesWithTitles[2]} caret={caret}/>
