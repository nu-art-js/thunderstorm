...
export class Example_LineGraph extends ComponentSync<Props, {
    page: number;
    index: number;
}> {
    ...
    private dataPoints = (data: Coordinates[], color: string) => data.map((d, i) => this.props.renderPoints ? this.props.renderPoints({
        ...
        (<circle onMouseEnter={(e) => {
            }} key={i} r={5} cx={this.xScale()(d.x + this.props.frequency)} cy={this.yScale()(d.y)} style={{ fill: color }}/>));
    private lines = (color: string, data: Coordinates[]) => {
        const lineArray = [];
        if (data.length > 1)
            for (let i = 0; i < data.length - 1; i++) {
                this.props.renderLines ? lineArray.push(this.props.renderLines({ x: this.xScale()(data[i].x + this.props.frequency), y: this.yScale()(data[i].y) }, { x: this.xScale()(data[i].x + this.props.frequency), y: this.yScale()(data[i].y) })) :
                    lineArray.push(<line key={i} x1={this.xScale()(data[i].x + this.props.frequency)} x2={this.xScale()(data[i + 1].x + this.props.frequency)} y1={this.yScale()(data[i].y)} y2={this.yScale()(data[i + 1].y)} strokeWidth={3} stroke={color}/>);
            }
        return lineArray;
    };
    private baseLine = (baseValue: number, color: string) => <line x1={this.xScale()(this.minAndMax().minX)} x2={this.width} y1={this.yScale()(baseValue)} y2={this.yScale()(baseValue)} strokeWidth={3} stroke={color} strokeDasharray={4} opacity={0.5}/>;
    w = 600;
    h = 300;
    margin = {
        top: 40,
        ...
        return <>
			<svg width={this.w} height={this.h} style={{ float: 'left', overflow: 'visible' }}>
				<g transform={`translate(${this.margin.left},${this.margin.top})`}>
					<AxisLeft yScale={this.yScale()} width={this.width + 100} axisPoint={this.xScale()(this.minAndMax().minX)}/>
					{this.props.renderBottomAxis ? this.props.renderBottomAxis(this.xScale(), this.height) :
                <AxisBottom xScale={this.xScale()} height={this.height} width={this.width + 100} tickValues={this.props.axesLabels?.x} borderBoxValues={this.props.borderBoxValues} paginated={true} frequency={this.props.frequency} axisPoint={this.minAndMax().minX} viewBox={this.props.xDomain} shiftData={true}/>} {this.props.data.map(_data => this.lines(_data.color, this.batchAllData(_data.data)[this.state.page]))}
