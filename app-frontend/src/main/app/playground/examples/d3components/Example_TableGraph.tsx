...
export class Example_TableGraph extends ComponentSync<Props, {}> {
    ...
    yScale = () => scaleLinear()
        ...
        .range([this.height, 0]);
    plots = (data: Coordinates[], color: string, icon?: ReactNode) => data.map((d, i) => <svg key={i} x={this.xScale()(d.x)} y={this.yScale()(d.y) + this.midY() / 2} style={{ overflow: 'visible' }}>{icon}</svg>);
    render() {
        return <>
			<svg width={this.w} height={this.h} style={{ float: 'left' }}>
				<g transform={`translate(${this.margin.left},${this.margin.top})`}>
					<AxisLeft yScale={this.yScale()} width={this.xScale()(this.props.xDomain ? this.props.xDomain + 1 : 8)} ticks={this.minAndMax().maxY + 1} tickValues={this.props.axesLabels.y} placeInMiddle={true} tickLines={true}/>
					<AxisBottom xScale={this.xScale()} frequency={this.props.frequency} height={this.height} tickValues={this.props.axesLabels?.x} axisPoint={0} viewBox={7}/>
