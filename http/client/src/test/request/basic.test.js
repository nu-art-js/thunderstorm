 ...
 */
import { HttpMethod } from '../../main/index.js';
...
const runTestCase = (testCase) => () => runSingleTestCase(test, testCase);
describe('HttpRequest - Basic Methods', () => {
    const client = createTestClient();
    it('GET request with query parameters', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.GET, '/get'),
    ...
    it('POST request with JSON body', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.POST, '/post'),
    ...
    it('PUT request with body', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.PUT, '/put'),
    ...
    it('PATCH request with body', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.PATCH, '/patch'),
    ...
    it('DELETE request', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.DELETE, '/delete')
        },
        result: async (actual) => {
            expect(actual.status).to.equal(200);
        }
    ...
    it('HEAD request', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.HEAD, '/get')
        },
        result: async (actual) => {
            expect(actual.status).to.equal(200);
    ...
    it('OPTIONS request', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.OPTIONS, '/get')
        },
        result: async (actual) => {
            expect(actual.status).to.equal(200);
        }
