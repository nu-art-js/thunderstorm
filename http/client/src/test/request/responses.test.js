 ...
 */
import { HttpException, HttpMethod } from '../../main/index.js';
...
describe('HttpRequest - Response Types', () => {
    ...
    it('Text response', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.GET, '/robots.txt'),
    ...
    it('Binary/Blob response', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.GET, '/bytes/100'),
    ...
    it('Stream response', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.GET, '/stream/5'),
    ...
    it('Large JSON response', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.GET, '/json')
        },
        result: async (actual) => {
            expect(actual.status).to.equal(200);
            expect(actual.data).to.be.an('object');
    ...
    it('Response with custom headers', runTestCase({
        input: {
            client,
            apiDef: createTestApiDef(HttpMethod.GET, '/response-headers'),
