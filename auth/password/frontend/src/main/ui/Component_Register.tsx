...
export class Component_Register
	...
	protected deriveStateFromProps(nextProps: Props<Request_RegisterAccount>, state: State<Request_RegisterAccount>) {
		...
		if (state.renderPasswordRules) {
			state.passwordAssertionConfig = ModuleFE_PasswordAuth.passwordAssertionConfig();
	...
	private onValueChanged = (value: string, id: keyof Request_RegisterAccount, onAccept: boolean = false) => {
	...
	private registerClicked = async () => {
		...
		try {
			await ModuleFE_PasswordAuth.registerAccount({
				...this.state.data,
