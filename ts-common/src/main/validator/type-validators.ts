...
export const tsValidateUnion = <T extends any>(validators: ValidatorTypeResolver<T>[], mandatory = true) => {
	return [tsValidateExists(mandatory), (input?: any) => {
		...
		return filterInstances(results).length !== 0 ? ['Input does not match any of the possible types', results] as InvalidResultArray<T>[] : undefined;
	}];
};
