...
test('debug: load page and capture errors', async ({page}) => {
	...
	await page.addInitScript(() => {
		...
		window.onerror = (msg: string, source: string, lineno: number, colno: number) => {
			(window as any).__capturedErrors.push({msg, source, lineno, colno});
			return false;
		};
	});
	...
	const hasDbApi = await page.evaluate(() => (window as any).DbApiFrontend !== undefined);
