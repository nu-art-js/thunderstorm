...
test.describe('BaseDB - sync', () => {
    ...
    test('onEntriesUpdated updates IDB and cache', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            return { before, after, item: after.find((x) => x._id === '1') };
        });
    ...
    test('onEntriesDeleted removes from IDB and cache', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            return { length: all.length, ids: all.map((x) => x._id) };
        });
    ...
    test('onQueryReturned with toDelete removes items from cache and IDB', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const all = api.cache.all();
            const ids = all.map((x) => x._id);
            await api.loadCache();
            const afterReload = api.cache.all().map((x) => x._id);
...
//# sourceMappingURL=sync.test.playwright.js.map