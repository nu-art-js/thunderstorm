...
test.describe('BaseDB - dispatcher', () => {
    ...
    test('onEntriesUpdated dispatches to module and UI', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const uiEvents = [];
            const spy = {
                dispatchModule(event, item) { moduleEvents.push({ event, item }); },
                dispatchUI(event, item) { uiEvents.push({ event, item }); },
                dispatchAll(event, item) { moduleEvents.push({ event, item }); uiEvents.push({ event, item }); }
            };
        ...
        expect(result.moduleEvents.map((e) => e.event)).toContain('upsert-all');
    });
    test('onEntryDeleted dispatches when delete completes', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const moduleEvents = [];
            const spy = {
                dispatchModule(event, _item) { moduleEvents.push({ event }); },
                dispatchUI(event, _item) { moduleEvents.push({ event }); },
                dispatchAll(_event, _item) { }
            };
            const deleted = { _id: '1', name: 'a', __created: 1, __updated: 1, _v: 'v1' };
            const client = new HttpClient();
            client.setConfig({ origin: 'http://127.0.0.1' });
            client.sendRequest = async () => ({ data: deleted, status: 200, statusText: 'OK', headers: {}, config: {} });
            const api = new TestBaseApi(client);
            await api.init();
            api.setDispatcher(spy);
        ...
        expect(result.moduleEvents.some((e) => e.event === 'delete')).toBe(true);
    });
    test('onQueryReturned dispatches when query completes', async ({ page }) => {
        ...
        expect(result.moduleEvents.some((e) => e.event === 'query')).toBe(true);
    });
    test('upsert without _id dispatches create', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const client = new HttpClient();
            client.setConfig({ origin: 'http://127.0.0.1' });
            client.sendRequest = async () => ({ data: created, status: 200, statusText: 'OK', headers: {}, config: {} });
            const api = new TestBaseApi(client);
            await api.init();
            api.setDispatcher(spy);
        ...
        expect(result.moduleEvents.some((e) => e.event === 'create')).toBe(true);
    });
    test('upsert with _id dispatches update', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const client = new HttpClient();
            client.setConfig({ origin: 'http://127.0.0.1' });
            client.sendRequest = async () => ({ data: updated, status: 200, statusText: 'OK', headers: {}, config: {} });
            const api = new TestBaseApi(client);
            await api.init();
            api.setDispatcher(spy);
        ...
        expect(result.moduleEvents.some((e) => e.event === 'update')).toBe(true);
    });
    test('patch dispatches patch event', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const client = new HttpClient();
            client.setConfig({ origin: 'http://127.0.0.1' });
            client.sendRequest = async () => ({ data: patched, status: 200, statusText: 'OK', headers: {}, config: {} });
            const api = new TestBaseApi(client);
            await api.init();
            api.setDispatcher(spy);
        ...
        expect(result.moduleEvents.some((e) => e.event === 'patch')).toBe(true);
    });
    test('queryUnique dispatches unique event', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const client = new HttpClient();
            client.setConfig({ origin: 'http://127.0.0.1' });
            client.sendRequest = async () => ({ data: item, status: 200, statusText: 'OK', headers: {}, config: {} });
            const api = new TestBaseApi(client);
            await api.init();
            api.setDispatcher(spy);
        ...
        expect(result.moduleEvents.some((e) => e.event === 'unique')).toBe(true);
    });
});
//# sourceMappingURL=dispatcher.test.playwright.js.map