...
test.describe('Integration - full CRUD flow', () => {
        ...
        const result = await page.evaluate(async () => {
            ...
            client.sendRequest = async (req) => {
                ...
                if (path.includes('query'))
                    return { data: [{ _id: '1', name: 'a', __created: 1, __updated: 1, _v: 'v1' }], status: 200, statusText: 'OK', headers: {}, config: {} };
                if (path.includes('upsert'))
                    return { data: { _id: '1', name: 'upserted', __created: 1, __updated: 2, _v: 'v1' }, status: 200, statusText: 'OK', headers: {}, config: {} };
                if (path.includes('patch'))
                    return { data: { _id: '1', name: 'patched', __created: 1, __updated: 3, _v: 'v1' }, status: 200, statusText: 'OK', headers: {}, config: {} };
                if (path.includes('delete-unique'))
                    return { data: { _id: '1', name: 'patched', __created: 1, __updated: 3, _v: 'v1' }, status: 200, statusText: 'OK', headers: {}, config: {} };
            ...
            const afterUpsert = api.cache.all().find((x) => x._id === '1')?.name;
            await api.patch({ _id: '1', name: 'patched' });
            const afterPatch = api.cache.all().find((x) => x._id === '1')?.name;
    ...
    test('IDB persistence: seed, loadCache, clear cache, loadCache again returns same data', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const client = new HttpClient();
            client.setConfig({ origin: 'http://127.0.0.1' });
            client.sendRequest = async () => ({ data: null, status: 200, statusText: 'OK', headers: {}, config: {} });
            const api = new TestBaseApi(client);
            await api.init();
            const seed = [
            ...
            return { afterFirst, afterClear, length: all.length, names: all.map((x) => x.name).sort() };
        });
...
//# sourceMappingURL=full-crud-flow.test.playwright.js.map