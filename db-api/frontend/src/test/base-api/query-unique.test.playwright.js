/*
 * @nu-art/db-api-frontend - Database API infrastructure for Thunderstorm frontend
 * Copyright (C) 2026 Adam van der Kruk aka TacB0sS
 * Licensed under the Apache License, Version 2.0
 */
import { expect, test } from '@playwright/test';
const testPagePath = '/src/test/index.html';
test.describe('BaseApi - queryUnique', () => {
    test.beforeEach(async ({ page }) => {
        await page.goto(testPagePath);
        await page.waitForFunction(() => window.DbApiFrontend !== undefined);
        await page.evaluate(() => window.DbApiFrontend.cleanupDbApiIDB());
    });
    test('queryUnique(params) and handler behaviour', async ({ page }) => {
        const result = await page.evaluate(async () => {
            const { TestBaseApi, HttpClient } = window.DbApiFrontend;
            const response = { _id: '1', name: 'unique', __created: 1, __updated: 1, _v: 'v1' };
            const client = new HttpClient();
            client.setConfig({ origin: 'http://127.0.0.1' });
            client.sendRequest = async () => ({ data: response, status: 200, statusText: 'OK', headers: {}, config: {} });
            const api = new TestBaseApi(client);
            await api.init();
            await api.queryUnique({ _id: '1' });
            const all = api.cache.all();
            return { length: all.length, item: all.find((x) => x._id === '1'), items: all };
        });
        console.log('query-unique items:', JSON.stringify(result.items));
        expect(result.length).toBe(1);
        expect(result.item?.name).toBe('unique');
    });
});
//# sourceMappingURL=query-unique.test.playwright.js.map