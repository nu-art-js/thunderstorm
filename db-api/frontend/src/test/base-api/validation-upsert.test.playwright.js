...
test.describe('BaseApi - validation on upsert', () => {
    ...
    test('upsert with failing validator throws and cache unchanged', async ({ page }) => {
        const result = await page.evaluate(async () => {
            const { TestBaseApiValidation, HttpClient } = window.DbApiFrontend;
            const client = new HttpClient();
            client.setConfig({ origin: 'http://127.0.0.1' });
            client.sendRequest = async () => ({ data: { _id: '1', name: 'x', __created: 1, __updated: 1, _v: 'v1' }, status: 200, statusText: 'OK', headers: {}, config: {} });
...
//# sourceMappingURL=validation-upsert.test.playwright.js.map