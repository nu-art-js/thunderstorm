...
test.describe('runSerializedById', () => {
    ...
    test('delete-wins: throws when upsert or patch run while delete is running or pending for same id', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            await api.init();
            const id = 'del-1';
            const deletePromise = api.runSerializedByIdExposed(id, 'delete', () => new Promise((r) => setTimeout(r, 50)));
    ...
    test('queueing: second call for same id runs after first completes; order is deterministic', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const id = 'q-1';
            const order = [];
            const delay = (ms, n) => () => new Promise((res) => {
                setTimeout(() => {
                    order.push(n);
                    res();
                }, ms);
    ...
    test('only one pending: third call for same id while one running and one pending rejects', async ({ page }) => {
        const result = await page.evaluate(async () => {
            ...
            const id = 'pending-1';
            let resolveOp1;
            const op1Promise = new Promise((r) => { resolveOp1 = r; });
...
//# sourceMappingURL=run-serialized-by-id.test.playwright.js.map