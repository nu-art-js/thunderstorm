...
describe('ModuleBE_BaseApi', () => {
	describe('query', () => {
		...
		it('returns items matching query after upserts', async () => {
			const {api} = setupApi();
			await api.upsert({_id: id('id1'), name: 'A', _v: 'v1'});
			await api.upsert({_id: id('id2'), name: 'B', _v: 'v1'});
			const items = await api.query({where: {}});
		...
		it('filters by where clause', async () => {
			const {api} = setupApi();
			await api.upsert({_id: id('id1'), name: 'A', tag: 'x', _v: 'v1'});
			await api.upsert({_id: id('id2'), name: 'B', tag: 'y', _v: 'v1'});
			await api.upsert({_id: id('id3'), name: 'C', tag: 'x', _v: 'v1'});
	...
	describe('queryUnique', () => {
		it('returns item when _id exists', async () => {
			const {api} = setupApi();
			await api.upsert({_id: id('id1'), _v: 'v1'});
			const item = await api.queryUnique({_id: id('id1')});
		...
		it('throws ApiException 404 when _id does not exist', async () => {
			const {api} = setupApi();
			try {
				await api.queryUnique({_id: id('nonexistent')});
		...
		it('404 message includes entity name', async () => {
			const {api} = setupApi();
			try {
				await api.queryUnique({_id: id('missing')});
				expect.fail('expected ApiException');
			} catch (e) {
				expect(e).to.be.instanceOf(ApiException);
	...
	describe('upsert', () => {
		it('saves item and returns it', async () => {
			const {api} = setupApi();
			const body = {_id: id('id1'), name: 'Test', _v: 'v1'};
			const result = await api.upsert(body);
			expect(result._id).to.equal('id1');
			const found = await api.queryUnique({_id: id('id1')});
			expect(found).to.deep.include({_id: id('id1')});
		});

		it('overwrites existing item when same _id', async () => {
			const {api} = setupApi();
			await api.upsert({_id: id('id1'), name: 'First', _v: 'v1'});
			const result = await api.upsert({_id: id('id1'), name: 'Second', _v: 'v1'});
	...
	describe('upsertAll', () => {
		...
		it('single item array works', async () => {
			const {api} = setupApi();
			const result = await api.upsertAll([{_id: id('only'), _v: 'v1'}]);
	...
	describe('delete', () => {
		it('removes item and returns it', async () => {
			const {api} = setupApi();
			await api.upsert({_id: id('id1'), _v: 'v1'});
			const deleted = await api.delete({_id: id('id1')});
			expect(deleted).to.not.be.undefined;
			expect(deleted!._id).to.equal('id1');
			try {
				await api.queryUnique({_id: id('id1')});
		...
		it('returns undefined when _id does not exist', async () => {
			const {api} = setupApi();
			const deleted = await api.delete({_id: id('nonexistent')});
	...
	describe('deleteQuery', () => {
		...
		it('deletes matching items and returns them', async () => {
			const {api} = setupApi();
			await api.upsert({_id: id('id1'), tag: 'x', _v: 'v1'});
			await api.upsert({_id: id('id2'), tag: 'x', _v: 'v1'});
			await api.upsert({_id: id('id3'), tag: 'y', _v: 'v1'});
		...
		it('returns empty array when no items match where', async () => {
			const {api} = setupApi();
			await api.upsert({_id: id('id1'), tag: 'a', _v: 'v1'});
	...
	describe('deleteAll', () => {
		it('deletes all items and returns them', async () => {
			const {api} = setupApi();
			await api.upsert({_id: id('id1'), _v: 'v1'});
			await api.upsert({_id: id('id2'), _v: 'v1'});
	...
	describe('createApisForDBModule', () => {
		it('creates working api with default CrudApiDef', async () => {
			...
			await api.upsert({_id: id('x'), _v: 'v1'} as any);
			const items = await api.query({where: {}});
	...
	describe('round-trip', () => {
		it('create, query, upsert again, query, delete, then 404', async () => {
			const {api} = setupApi();
			await api.upsert({_id: id('r1'), name: 'Initial', _v: 'v1'});
			let items = await api.query({where: {}});
			expect(items).to.have.length(1);
			expect(items[0].name).to.equal('Initial');

			await api.upsert({_id: id('r1'), name: 'Updated', _v: 'v1'});
			items = await api.query({where: {}});
			expect(items).to.have.length(1);
			expect(items[0].name).to.equal('Updated');

			const deleted = await api.delete({_id: id('r1')});
			expect(deleted!._id).to.equal('r1');
			items = await api.query({where: {}});
			expect(items).to.deep.equal([]);

			try {
				await api.queryUnique({_id: id('r1')});
				expect.fail('expected 404');
			} catch (e) {
				expect(e).to.be.instanceOf(ApiException);
				expect((e as ApiException).responseCode).to.equal(404);
