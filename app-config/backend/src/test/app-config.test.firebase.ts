/*
 * @nu-art/app-config-backend - Firebase tests for ModuleBE_AppConfigDB and getConfigByKey
...
import {stormTester, StormTestInput} from '../../../../thunderstorm/backend/src/main/test/StormTest.js';
...
import {DefaultStormTestConfig_AppConfig} from './utils/helpers.js';
...
const test_GetConfigByKeyApi = async (input: Input_GetConfigByKeyApi): Promise<Result_GetConfigByKeyApi> => {
	new AppConfigKey_BE(input.key, async () => input.resolverValue);
	const fromApi = await ModuleBE_AppConfigAPI.getConfigByKey({key: input.key});
	const fromDb = await ModuleBE_AppConfigDB.getResolverDataByKey(input.key);
	expect(fromApi).to.deep.equal(fromDb);
	return fromApi;
};
...
const DefaultStormTest: StormTestInput = {
	...DefaultStormTestConfig_AppConfig,
};

describe('App-config - getResolverDataByKey', () => {
	it('throws 404 when key is not registered', async () => {
		await stormTester(DefaultStormTest, run_GetResolver404({
...
describe('App-config - registerKey and getAppKey', () => {
	it('returns resolver value and persists', async () => {
		await stormTester(DefaultStormTest, run_RegisterAndGet({
...
describe('App-config - setAppKey and getAppKey', () => {
	it('set then get returns set value', async () => {
		await stormTester(DefaultStormTest, run_SetAndGet({
...
describe('App-config - createDefaults', () => {
	it('creates entries for all registered keys', async () => {
		await stormTester(DefaultStormTest, run_CreateDefaults({
...
describe('App-config - getConfigByKey API', () => {
	it('returns same as getResolverDataByKey', async () => {
		await stormTester(DefaultStormTest, run_GetConfigByKeyApi({
