...
export class ATS_AccountEditor extends ComponentSync<Props, State> {
...
class Component_AccountList extends ComponentSync<ListProps, ListState> implements OnAccountsUpdated {
    ...
    render() {
        return <LL_V_L className={'form-container account-list'}>
			...
			<LL_V_L className={'match_width users-list'}>
                ...
                return <TS_PropRenderer.Horizontal onClick={() => this.props.setSelectedAccount(account)} key={generateUUID()} className={className} label={account.email}/>;
            })}
			</LL_V_L>
		</LL_V_L>;
    }
