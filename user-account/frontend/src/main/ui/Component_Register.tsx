...
export class Component_Register extends ComponentSync<Props<Request_RegisterAccount>, State<Request_RegisterAccount>> {
    // ######################### Lifecycle #########################
        ...
        return <LL_V_C className="ts-account__authenticate">
			{_keys(form).map((key, i) => {
                ...
                return <TS_PropRenderer.Vertical label={field.label} key={i}>
						<TS_InputV2 id={key} value={data[key]} type={field.type} allowAccept={true} onChange={(data) => this.onValueChanged(data, key)} onAccept={(data) => this.onValueChanged(data, key, true)}/>
						{key === 'password' && this.renderPasswordRules()}
					</TS_PropRenderer.Vertical>;
            })}
			{this.renderErrorMessages()}
			<Button onClick={this.registerClicked} actionInProgress={this.state.submitting} className={`clickable ts-account__action-button`} disabled={!!this.state.passwordFailureReport}>Register</Button>
		</LL_V_C>;
    }
