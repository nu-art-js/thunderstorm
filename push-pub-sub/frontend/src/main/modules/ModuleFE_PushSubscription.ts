...
const pushSubscriptionDispatcher: EventDispatcher<(DBProto_PushSubscription & DB_Prototype)['dbType']> = (...params) => {
...
const pushSubscriptionConfig = {
	...
	uniqueKeys: (DBDef_PushSubscription.uniqueKeys ?? ['_id']) as (DBProto_PushSubscription & DB_Prototype)['uniqueKeys'],
	...
	dbConfig: {
		...
		uniqueKeys: (DBDef_PushSubscription.uniqueKeys ?? ['_id']) as (keyof (DBProto_PushSubscription['dbType']))[]
	}
};

export class ModuleFE_PushSubscription_Class
	extends ModuleFE_BaseApi<DBProto_PushSubscription & DB_Prototype> {

	constructor() {
		super({
			config: pushSubscriptionConfig,
			crudApiDef: CrudApiDef<DBProto_PushSubscription & DB_Prototype>(DBDef_PushSubscription.dbKey),
