...
export class TS_FloatingWindows extends ComponentSync<unknown, State> implements FloatingWindows_WindowsUpdated {
    //######################### Life Cycle #########################
    ...
    render() {
        ...
        return <div id={'ts-floating-windows'}>
			{this.state.windowModels.map(model => {
                return <TS_FloatingWindow key={model.key} model={model}/>;
            })}
		</div>;
    }
}
