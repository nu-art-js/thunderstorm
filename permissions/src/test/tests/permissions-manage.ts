...
export function createApi() {
	...
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionApi.upsert({projectId: testConfig1.project._id, path: apiPath, accessLevelIds: []});
...
export function failedCreateApi() {
	...
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionApi.upsert({projectId: testConfig1.project._id, path: wrongApiPath, accessLevelIds: []});
...
export function checkAccessLevelsPropertyOfGroup() {
	...
	scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey1));
	scenario.add(__custom(async (action, data) => {
		const group = await ModuleBE_PermissionGroup.upsert({label: 'test group one', accessLevelIds: [data.level._id]});
...
export function checkUpdateOfGroupAccessLevelsProperty() {
	...
	scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey1));
	scenario.add(__custom(async (action, data) => {
		const group = await ModuleBE_PermissionGroup.upsert({label: 'test group one', accessLevelIds: [data.level._id], _id: uniqId1});
...
export function checkUpdateOfGroupAccessLevelsPropertyToHigherValue() {
	...
	scenario.add(__custom(async (action, data) => {
		const group = await ModuleBE_PermissionGroup.upsert({label: 'test group one', accessLevelIds: [data.level._id], _id: uniqId1});
		const higherValueLevel = await ModuleBE_PermissionAccessLevel.upsert({...testLevel2, domainId: data.level.domainId});
...
export function checkPatchOfGroupAccessLevelsProperty() {
	...
	scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey1));
	scenario.add(__custom(async (action, data) => {
		const group = await ModuleBE_PermissionGroup.upsert({label: 'test group one', accessLevelIds: [data.level._id], _id: uniqId1, customFields: []});
		group.accessLevelIds = [];
...
export function checkGroupAccessLevelsAfterUpdatingLevelDocument() {
	...
	scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey1));
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionGroup.upsert({label: 'test group one', accessLevelIds: [data.level._id], _id: uniqId1});
		const newLevelValue = 353;
		data.level.value = newLevelValue;
		await ModuleBE_PermissionAccessLevel.upsert(data.level);
...
export function checkGroupAccessLevelsAfterPatchingLevelDocument() {
	...
	scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey1));
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionGroup.upsert({label: 'test group one', accessLevelIds: [data.level._id], _id: uniqId1, customFields: []});
		const newLevelValue = 353;
		data.level.value = newLevelValue;
		await ModuleBE_PermissionAccessLevel.patch(data.level);
		const group = await ModuleBE_PermissionGroup.queryUnique({_id: uniqId1});
...
export function createTowGroups() {
	...
	scenario.add(cleanup());
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionGroup.upsert({label: 'test group one', accessLevelIds: [uniqId3], _id: uniqId1});
		await ModuleBE_PermissionGroup.upsert({label: 'test group two', accessLevelIds: [], _id: uniqId2});
...
export function createUser() {
	...
	scenario.add(cleanup());
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionUser.upsert({accountId: uniqId1});
...
export function createUserWithGroups() {
	...
	scenario.add(cleanup());
	scenario.add(__custom(async (action, data) => {
		const group1 = await ModuleBE_PermissionGroup.upsert({label: 'test group one', accessLevelIds: [uniqId3]});
		const group2 = await ModuleBE_PermissionGroup.upsert({label: 'test group two', accessLevelIds: []});
		await ModuleBE_PermissionUser.upsert({
			accountId: uniqId1,
			groups: [{groupId: group1._id, customField: {unit: 'eq1'}}, {groupId: group2._id, customField: {unit: 'eq2'}}]
		});
...
export function createTowUsers() {
	...
	scenario.add(__custom(async (action, data) => {
		...
		const group1 = await ModuleBE_PermissionGroup.upsert({label: 'test group one'});
		const group2 = await ModuleBE_PermissionGroup.upsert({label: 'test group two'});
		await ModuleBE_PermissionUser.upsert({accountId: userUuid1, groups: [], _id: uniqId1});
		await ModuleBE_PermissionUser.upsert({accountId: userUuid2, groups: [{groupId: group1._id, customField}, {groupId: group2._id, customField}]});
...
export function checkCreateUserWithEmptyGroups() {
	...
	scenario.add(cleanup());
	scenario.add(__custom(async (action, data) => {
		const user = await ModuleBE_PermissionUser.upsert({accountId: userUuid1, groups: [], _id: uniqId1});
...
export function checkCreateUserWithGroups() {
	...
	scenario.add(__custom(async (action, data) => {
		...
		const group1 = await ModuleBE_PermissionGroup.upsert({label: 'test group one'});
		const group2 = await ModuleBE_PermissionGroup.upsert({label: 'test group two'});
		const user = await ModuleBE_PermissionUser.upsert({accountId: userUuid2, groups: [{groupId: group1._id, customField}, {groupId: group2._id, customField}]});
...
export function checkUpdatedUserGroups() {
	...
	scenario.add(__custom(async (action, data) => {
		...
		const user = await ModuleBE_PermissionUser.upsert({accountId: userUuid2, groups});
		if (!user.__groupIds || user.__groupIds.length !== 2)
			throw new TestException('__groupIds property didn\'t created');

		const group3 = await ModuleBE_PermissionGroup.upsert({label: 'test group three'});
		groups.push({groupId: group3._id, customField});
		const updatedUser = await ModuleBE_PermissionUser.upsert({...user, groups});
...
// 	scenario.add(cleanup());
// 	scenario.add(__custom(async (action, data) => {
// 		await ModuleBE_PermissionUser.upsert({accountId: userUuid1, groups: [{groupId: uniqId2, customField: {unit: "eq1"}}, {groupId: uniqId2, customField: {unit: "eq2"}}], _id: uniqId1});
...
export function failedCreateUserWithDuplicateGroups() {
	...
	scenario.add(__custom(async (action, data) => {
		const customField = {unit: 'eq1'};
		const group = await ModuleBE_PermissionGroup.upsert({label: 'test group'});
		await ModuleBE_PermissionUser.upsert({
			accountId: userUuid1,
			groups: [{groupId: group._id, customField: customField}, {groupId: group._id, customField: customField}]
		});
...
export function failedCreateUserWithDuplicateGroupsButOneUndefinedCFAndOtherEmptyObj() {
	...
	scenario.add(__custom(async (action, data) => {
		const group = await ModuleBE_PermissionGroup.upsert({label: 'test group'});
		await ModuleBE_PermissionUser.upsert({accountId: userUuid1, groups: [{groupId: group._id, customField: undefined}, {groupId: group._id, customField: {}}]});
	}).setLabel('Fail to bind duplicate groups (with the same groupId && customField - one undefined and the other empty Obj) to user, as expected')
		.expectToFail(ApiException));
	return scenario;
}
...
export function failedDeleteGroupAssociatedToUser() {
	...
	scenario.add(__custom(async (action, data) => {
		...
		await ModuleBE_PermissionUser.upsert({
			accountId: userUuid1,
			groups: [{groupId: group._id, customField: {unit: 'eq1'}}, {groupId: group._id, customField: {unit: 'eq2'}}]
		});
		await ModuleBE_PermissionGroup.deleteUnique(group._id);
...
export function createUserWithDuplicateGroupIdButDifferentCustomField() {
	...
	scenario.add(__custom(async (action, data) => {
		...
		await ModuleBE_PermissionUser.upsert({
			accountId: userUuid1,
			groups: [{groupId: group._id, customField: {unit: 'eq2', test: 'test1'}}, {groupId: group._id, customField: {unit: 'eq2'}}]
		});
...
export function createGroupWithLegalCustomField() {
	...
	scenario.add(cleanup());
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionGroup.upsert({label: 'test group one', customFields: [{UnitId: generateHex(500), Production: 'false'}], _id: uniqId1});
...
export function failToCreateGroupWithIllegalCustomField() {
	...
	scenario.add(cleanup());
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionGroup.upsert({label: 'test group one', customFields: [{UnitId: generateHex(501), Production: 'false'}], _id: uniqId1});
...
export function failedCreateTwoGroupsWithSameName() {
	...
	scenario.add(cleanup());
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionGroup.upsert({label: 'test group', accessLevelIds: [uniqId3], _id: uniqId1});
		await ModuleBE_PermissionGroup.upsert({label: 'test group', accessLevelIds: [], _id: uniqId2});
...
export function failedCreateGroupWithDuplicateAccessLevel() {
	...
	scenario.add(cleanup());
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionGroup.upsert({label: 'test', accessLevelIds: [uniqId3, uniqId3], _id: uniqId1});
...
export function checkInsertUserIfNotExist() {
	...
	scenario.add(__custom(async (action, data) => {
		...
		await ModuleBE_PermissionUser.queryUnique({accountId: account._id});
...
export function createApiWithAccessLevel() {
	...
	scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey));
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionApi.upsert({path: apiPath, accessLevelIds: [data.level._id], projectId: data.project._id});
...
export function tryDeleteDomainAssociatedWithAccessLevel() {
	...
	scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey1));
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionDomain.deleteUnique(data.domain._id);
...
export function tryDeleteAccessLevelAssociatedWithGroup() {
	...
	scenario.add(__custom(async (action, data) => {
		...
		await ModuleBE_PermissionGroup.upsert({_id: uniqId1, accessLevelIds: [levelId], label: 'group-test'});
		await ModuleBE_PermissionAccessLevel.deleteUnique(levelId);
...
export function tryDeleteAccessLevelAssociatedWithApi() {
	...
	scenario.add(__custom(async (action, data) => {
		const levelId = data.level._id;
		await ModuleBE_PermissionApi.upsert({_id: uniqId1, accessLevelIds: [levelId], projectId: data.project._id, path: apiPath});
		await ModuleBE_PermissionAccessLevel.deleteUnique(levelId);
...
export function checkDeleteAccessLevelsDocument() {
	...
	scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey1));
	scenario.add(__custom(async (action, data) => {
		await ModuleBE_PermissionAccessLevel.deleteUnique(data.level._id);
