...
class DomainLevelRenderer extends ComponentSync<MultiSelect_Selector<string>, {
    domainId?: string;
}> {
    render() {
        ...
        return <LL_V_L className={'domain-level-renderer'}>
			<Permissions_DropDown.Domain selected={this.state.domainId} queryFilter={domain => availableDomainsIds.includes(domain._id)} onSelected={domain => this.setState({ domainId: domain._id })} sortBy={['namespace']} placeholder={'Domain'}/>
			<Permissions_DropDown.AccessLevel disabled={!this.state.domainId} queryFilter={level => this.state.domainId === level.domainId} selected={undefined} onSelected={(accessLevel) => this.props.onSelected(accessLevel._id)} sortBy={['value']} placeholder={'Level'}/>
		</LL_V_L>;
    }
...
class GroupRenderer extends ComponentSync<MultiSelect_Selector<{
    groupId: string;
}>> {
    ...
    render() {
        return <Permissions_DropDown.Group onSelected={group => this.props.onSelected({ groupId: group._id })} queryFilter={group => !this.props.existingItems.find(g => g.groupId === group._id)} sortBy={['label']} selected={undefined}/>;
    }
}
...
const MultiSelect_PermissionsAccessLevel: StaticProps_TS_MultiSelect_V2<string> = {
    ...
    selectionRenderer: (props) => <DomainLevelRenderer onSelected={props.onSelected} className={props.className} existingItems={props.existingItems} queryFilter={props.queryFilter}/>,
...
const MultiSelect_PermissionsAccessLevelStam: StaticProps_TS_MultiSelect_V2<string> = {
    ...
    selectionRenderer: (selector) => <Permissions_DropDown.AccessLevel queryFilter={item => {
...
const MultiSelect_PermissionsGroup: StaticProps_TS_MultiSelect_V2<{
    groupId: string;
}> = {
    ...
    selectionRenderer: (props) => <GroupRenderer onSelected={props.onSelected} className={props.className} existingItems={props.existingItems} queryFilter={props.queryFilter}/>,
