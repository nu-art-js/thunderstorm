...
class Component_APIEditor extends Component_BasePermissionItemEditor<DBProto_PermissionAPI> {
    static defaultProps = {
    ...
    editorContent = () => {
        ...
        return <>
			...
			<TS_PropRenderer.Vertical label={'Access Levels'}>
				<MultiSelect.AccessLevel editable={api} prop={'accessLevelIds'} className={'domain-level-list'} itemRenderer={(levelId, onDelete) => {
...
class Controller_ApiEditor extends TS_EditableItemControllerProto<DBProto_PermissionAPI> {
    static defaultProps = {
        module: ModuleFE_PermissionAPI,
...
export class PermissionAPIEditor extends Page_ItemsEditor<DBProto_PermissionAPI, ProtoDef_Selection, Props> {
    //######################### Static #########################
