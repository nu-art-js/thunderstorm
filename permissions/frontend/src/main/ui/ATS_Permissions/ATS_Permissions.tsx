    ...
    render() {
        return <LL_V_L id={'dev-page__permissions'}>
			<LL_H_C className="match_width flex__space-between">{TS_AppTools.renderPageHeader('Permissions Editor')}
				<LL_H_C>
					<Button variant={'secondary'} disabled={this.state.creatingPermissions} className={'item-list__add-button'} onClick={this.toggleStrictMode}>{SessionKey_StrictMode_FE.get() ? 'Disable' : 'Enable'} Strict
						Mode</Button>
				</LL_H_C>
			</LL_H_C>
			<LL_H_C className={'links-container'}>
				<TS_NavLink key={PermissionProjectsEditor.Route.key} route={PermissionProjectsEditor.Route} ignoreClickOnSameRoute={true} className={({ isActive }) => _className('link-button', isActive && 'selected')}>
					Project
				</TS_NavLink>
				<TS_NavLink key={PermissionDomainsEditor.Route.key} route={PermissionDomainsEditor.Route} ignoreClickOnSameRoute={true} className={({ isActive }) => _className('link-button', isActive && 'selected')}>
					Domains
				</TS_NavLink>
				<TS_NavLink key={PermissionGroupsEditor.Route.key} route={PermissionGroupsEditor.Route} ignoreClickOnSameRoute={true} className={({ isActive }) => _className('link-button', isActive && 'selected')}>
					Groups
				</TS_NavLink>
				<TS_NavLink key={PermissionUsersEditor.Route.key} route={PermissionUsersEditor.Route} ignoreClickOnSameRoute={true} className={({ isActive }) => _className('link-button', isActive && 'selected')}>
					Users
				</TS_NavLink>
			</LL_H_C>
			<Outlet />
