import { _className, ComponentSync, LL_H_C, LL_V_L } from "@nu-art/thunder-routing/index";
...
export class PermissionKeysEditor extends ComponentSync<Props, State> {
    ...
    private renderKey = (item: PermissionKey_FE) => {
        ...
        return <div className={className} onClick={() => this.setState({ editedItem: item })} key={item.key}>{item.key}</div>;
    };
