...
import {generateHex, ThisShouldNotHappenException} from '@thunder-storm/common';
import {ModuleBE_Account} from '@thunder-storm/user-account/backend';
...
export function failedCreateApi() {
    ...
    scenario.add(__custom(async (action, data) => {
        await ModuleBE_PermissionApi.upsert({
            projectId: testConfig1.project._id,
            path: wrongApiPath,
            accessLevelIds: []
        });
...
export function checkAccessLevelsPropertyOfGroup() {
    ...
    scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey1));
    scenario.add(__custom(async (action, data) => {
        const group = await ModuleBE_PermissionGroup.upsert({
            label: 'test group one',
            accessLevelIds: [data.level._id]
        });
...
export function checkUpdateOfGroupAccessLevelsPropertyToHigherValue() {
    ...
    scenario.add(__custom(async (action, data) => {
        ...
        const higherValueLevel = await ModuleBE_PermissionAccessLevel.upsert({
            ...testLevel2,
            domainId: data.level.domainId
        });
        if (!group.accessLevelIds || !group.accessLevelIds.length) {
            throw new TestException('Didn\'t test-add-data the group properly');
        }
...
export function checkGroupAccessLevelsAfterUpdatingLevelDocument() {
    ...
    scenario.add(setupDatabase(testConfig1, testLevel1).setWriteKey(contextKey1));
    scenario.add(__custom(async (action, data) => {
        await ModuleBE_PermissionGroup.upsert({
            label: 'test group one',
            accessLevelIds: [data.level._id],
            _id: uniqId1
        });
...
export function createUserWithGroups() {
    ...
    scenario.add(__custom(async (action, data) => {
        ...
        await ModuleBE_PermissionUser.upsert({
            accountId: uniqId1,
            groups: [{groupId: group1._id, customField: {unit: 'eq1'}}, {
                groupId: group2._id,
                customField: {unit: 'eq2'}
            }]
        });
...
export function createTowUsers() {
    ...
    scenario.add(__custom(async (action, data) => {
        ...
        await ModuleBE_PermissionUser.upsert({
            accountId: userUuid2,
            groups: [{groupId: group1._id, customField}, {groupId: group2._id, customField}]
        });
...
export function checkCreateUserWithGroups() {
    ...
    scenario.add(__custom(async (action, data) => {
        ...
        const group1 = await ModuleBE_PermissionGroup.upsert({label: 'test group one'});
        const group2 = await ModuleBE_PermissionGroup.upsert({label: 'test group two'});
        const user = await ModuleBE_PermissionUser.upsert({
            accountId: userUuid2,
            groups: [{groupId: group1._id, customField}, {groupId: group2._id, customField}]
        });
...
export function failedCreateUserWithDuplicateGroupsButOneUndefinedCFAndOtherEmptyObj() {
    ...
    scenario.add(__custom(async (action, data) => {
        ...
        await ModuleBE_PermissionUser.upsert({
            accountId: userUuid1,
            groups: [{groupId: group._id, customField: undefined}, {groupId: group._id, customField: {}}]
        });
...
export function createUserWithDuplicateGroupIdButDifferentCustomField() {
    ...
    scenario.add(__custom(async (action, data) => {
        ...
        await ModuleBE_PermissionUser.upsert({
            accountId: userUuid1,
            groups: [{groupId: group._id, customField: {unit: 'eq2', test: 'test1'}}, {
                groupId: group._id,
                customField: {unit: 'eq2'}
            }]
        });
...
export function createGroupWithLegalCustomField() {
    ...
    scenario.add(__custom(async (action, data) => {
        await ModuleBE_PermissionGroup.upsert({
            label: 'test group one',
            customFields: [{UnitId: generateHex(500), Production: 'false'}],
            _id: uniqId1
        });
...
export function failToCreateGroupWithIllegalCustomField() {
    ...
    scenario.add(__custom(async (action, data) => {
        await ModuleBE_PermissionGroup.upsert({
            label: 'test group one',
            customFields: [{UnitId: generateHex(501), Production: 'false'}],
            _id: uniqId1
        });
...
export function checkInsertUserIfNotExistByExistUser() {
    ...
    }).setLabel('Skip test-add-data exist user'));
    return scenario;
}
