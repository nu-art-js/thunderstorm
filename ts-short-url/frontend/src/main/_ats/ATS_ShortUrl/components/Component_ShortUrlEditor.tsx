...
export class Component_ShortUrlEditor extends TS_EditableContent<DBProto_ShortUrl, Props, State> {
        ...
        return <TS_Card className={'short-url-editor'}>
			...
			<LL_H_C>
				...
				<TS_PropRenderer.Vertical label={'Title'}>
					<StringEditableInput editable={this.state.editable} prop={'title'} onChange={value => {
                if (value.length)
			...
			<TS_PropRenderer.Vertical label={'Full Url'}>
				<TextArea editable={this.state.editable} prop={'fullUrl'} className={'url-input'} onChange={value => {
                if (value.length)
			...
			<TS_PropRenderer.Vertical label={'Description'}>
				<TextArea editable={this.state.editable} prop={'description'} onChange={value => {
                if (value?.length)
			...
			<LL_H_C className={'buttons-container'}>
				<Button variant={'tertiary'} disabled={!_id} onClick={async () => {
