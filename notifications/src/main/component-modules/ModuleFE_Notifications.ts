...
export class ModuleFE_Notifications_Class
	...
	create(id?: string): UI_Notification {
		...
		const notification: DB_Notification = list.find((n: DB_Notification) => n._id === id) ?? {
			postDelayed: 5000,
	...
	private upsert(notification: DB_Notification) {
		...
		const index = notifications.findIndex((n: DB_Notification) => n._id === notification._id);
		if (index !== -1)
	...
	private showImpl(notification: DB_Notification, postDelay: number) {
		...
		const index = this.showing.findIndex((n: DB_Notification) => n._id === notification._id);
		if (index !== -1)
	...
	private delete(notification: DB_Notification) {
		...
		const list = this.notificationStorage.get([]) ?? [];
		this.notificationStorage.set(list.filter((item: DB_Notification) => item._id !== notification._id));
	}
