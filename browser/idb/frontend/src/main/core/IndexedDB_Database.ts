...
export class IndexedDB_Database
	...
	registerStore = (dbConfig: DBConfig<any>, onDBOpenCallback?: AsyncVoidFunction) => {
	...
	async getStore(config: DBConfig<any>, write = false, _store?: IDBObjectStore): Promise<IDBObjectStore> {
		if (_store)
			return _store;
