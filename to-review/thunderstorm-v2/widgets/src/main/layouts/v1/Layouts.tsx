...
class LinearLayout
	...
	render() {
		...
		return <div {...props} ref={innerRef} className={_className(this.layoutClass, props.className)}>
			{props.children}
		</div>;
	}
}
...
export class Grid
	...
	render() {
		const {innerRef, ...props} = this.props;
		return <div {...props} ref={innerRef} className={_className('ts-grid', props.className)}>
			{props.children}
		</div>;
	}
}
