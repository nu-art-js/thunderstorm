...
export class TS_GridTableProto<Proto extends DBProto<any>, EditorProps extends object = object>
	...
	render() {
		...
		const ItemController = (item: Proto['uiType'], index: number) => {
			return <TS_EditableItemController<Proto, EditorProps> module={module} editor={editor} autoSave={autoSave}
																														editorProps={{...editorProps, index} as EditorProps} onError={onError} key={item._id} item={item}/>;
		};
		const NewItemController = createInitialInstance &&
			<TS_EditableItemController<Proto, EditorProps> module={module} editor={editor} onError={onError} autoSave={autoSave} saveAction={saveAction}
																										 onSave={(item) => this.forceUpdate()} editorProps={editorProps} key={`new-${items.length}`}
